...
def distinct_sport_keys(raw_dir: str) -> dict[str, int]:
    """Count each ``activityType.typeKey`` across the archived activities files."""
    counts: dict[str, int] = {}
...
def main():
    ...
    keys = distinct_sport_keys(args.raw_dir)
    if not keys:
        print("  (没找到活动文件)")
